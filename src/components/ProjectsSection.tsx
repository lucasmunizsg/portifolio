        ...
        return (
            <div 
                ...
                className={`project-card cursor-pointer shrink-0 w-[290px] sm:w-[350px] md:w-[420px] mr-6 group relative aspect-[4/5] bg-[#131313]/80 backdrop-blur-md overflow-hidden border transition-all duration-700 ${
                    isActive 
                        ? 'border-violet-500 shadow-[0_0_30px_rgba(139,92,246,0.4)] z-10 scale-[1.02]' 
                        : 'border-white/5 hover:border-violet-500/30'
                }`}
            >
                {/* Animated Border SVG when active */}
                {isActive && (
                ...
                {/* Hover Border Accent */}
                <div className="absolute left-0 top-0 w-[2px] h-0 bg-violet-500 shadow-[0_0_15px_rgba(139,92,246,0.8)] group-hover:h-full transition-all duration-700 z-10"></div>
                ...
                <div className={`absolute bottom-10 left-10 right-10 transform transition-transform duration-500 z-10 ${
                    ...
                    <span className="font-label text-[10px] text-violet-400 uppercase tracking-[0.3em] mb-3 block font-bold" style={{ textShadow: '0 0 10px rgba(139,92,246,0.8)' }}>
                        Arquivo 0{index + 1}
                    </span>
                    <h3 className="font-display text-3xl font-black text-white tracking-tighter uppercase mb-6 leading-none group-hover:text-violet-50 transition-colors">
                        {project.title}
                    </h3>
                    ...
                    <div className={`flex flex-wrap gap-2 transition-all duration-700 delay-100 ${
                        ...
                        {project.technologies.map(tech => (
                            <span key={tech} className="px-3 py-1 bg-violet-500/10 border border-violet-500/20 text-[9px] uppercase tracking-widest text-violet-200 font-medium">
                                {tech}
                            </span>
                        ))}
                    </div>
                ...
                <div className={`absolute top-10 right-10 w-12 h-12 rounded-full bg-white/5 backdrop-blur-xl border flex items-center justify-center transition-all duration-500 z-10 ${
                    isActive 
                        ? 'opacity-100 border-violet-400 shadow-[0_0_15px_rgba(139,92,246,0.6)] rotate-45' 
                        : 'opacity-0 border-white/10 group-hover:opacity-100 group-hover:rotate-45'
                }`}>
                    <span className={`material-symbols-outlined text-xl transition-colors ${isActive ? 'text-violet-400' : 'text-white'}`}>arrow_outward</span>
                </div>
            </div>
        );
    };
    ...
    return (
        <section id="work" className="relative px-6 md:px-12 py-32 max-w-[1920px] mx-auto bg-[#0e0e0e] overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8 relative z-10">
                <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-4">
                        <span className="w-12 h-[1px] bg-violet-500/40"></span>
            ...
            <div className="overflow-hidden w-full relative">
                ...
                <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-violet-500/20 to-transparent z-0"></div>
                <div className="absolute bottom-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-violet-500/20 to-transparent z-0"></div>
