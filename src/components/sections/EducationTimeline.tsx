    ...
    return (
        ...
        >
            ...
            <div className={`absolute left-4 md:left-1/2 top-0 w-3 h-3 rounded-full transform -translate-x-1/2 md:-translate-x-1/2 border-2 transition-all duration-500 z-30 ${
                isInView ? 'bg-[#008000] border-[#008000] shadow-[0_0_15px_rgba(0,128,0,0.8)] scale-125' : 'bg-[#0e0e0e] border-zinc-600'
            } ${isActive ? 'scale-150' : 'group-hover:scale-150'}`}></div>

            {/* Connection Line from Timeline to Card (Desktop) */}
            <motion.div
                className={`hidden md:block absolute top-[5px] h-[2px] bg-[#008000] shadow-[0_0_10px_rgba(0,128,0,0.8)] z-20 ${index % 2 === 0 ? 'origin-right' : 'origin-left'}`}
                style={{
            ...
            <motion.div
                className={`block md:hidden absolute top-[5px] left-4 h-[2px] bg-[#008000] shadow-[0_0_10px_rgba(0,128,0,0.8)] z-20 origin-left`}
            ...
            <div onClick={onClick} className={`w-[calc(100%-2rem)] ml-8 md:ml-0 md:w-[46%] p-10 bg-[#131313]/80 backdrop-blur-md border cursor-pointer transition-all duration-700 relative overflow-hidden ${
                isActive ? 'border-[#008000]/50 shadow-[0_0_20px_rgba(0,128,0,0.2)] scale-[1.01]' : 'border-white/5 hover:border-[#008000]/30'
                ...
                <div className="flex flex-col gap-6 relative z-10">
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-3">
                            <span className="font-label text-[10px] text-[#008000] uppercase tracking-[0.2em] font-bold">
                                {study.year}
                            </span>
                        ...
                        <h3 className="font-display text-2xl md:text-3xl font-black text-white tracking-tighter uppercase leading-none group-hover:text-green-50 transition-all duration-500">
                            {study.course}
                        </h3>
                ...
                {/* Hover Border Accent */}
                <div className="absolute left-0 top-0 w-[2px] h-0 bg-[#008000] shadow-[0_0_10px_rgba(0,128,0,0.4)] group-hover:h-full transition-all duration-700"></div>
            </div>
        </div>
    );
};
...
const EducationTimeline: React.FC<EducationTimelineProps> = ({ education }) => {
    ...
    return (
        <div className="relative max-w-5xl mx-auto" ref={containerRef}>
            <BeamLine containerRef={containerRef} color="bg-[#008000]" glowColor="rgba(0,128,0,0.8)" />
